...
function CopilotView() {
    ...
    return (
        ...
        >
            <h2 className="mb-2 text-sm font-medium uppercase tracking-wider text-slate-400">AI Copilot</h2>
            <textarea
                className="min-h-[120px] w-full rounded-lg border border-slate-800 bg-slate-950 p-3 text-sm text-slate-300 placeholder-slate-600 outline-none transition-colors focus:border-amber-500/50"
                placeholder="Describe the code you want to generate..."
                onChange={(e) => setInput(e.target.value)}
            />
            <button
                className="flex w-full justify-center rounded-lg bg-amber-500 px-4 py-2.5 text-sm font-semibold uppercase tracking-wider text-slate-950 transition-all hover:bg-amber-400 disabled:cursor-not-allowed disabled:opacity-50"
            ...
            <div className="h-full w-full overflow-y-auto rounded-lg border border-slate-800 bg-slate-950 p-2">
                <ReactMarkdown
                    components={{
                        ...
                        code({ inline, className, children, ...props }: any) {
                            ...
                            return !inline ? (
                                <SyntaxHighlighter
                                    ...
                                    language={language}
                                    PreTag="pre"
                                    className="!m-0 !h-full !rounded-lg !bg-slate-950 !p-2"
                                >
