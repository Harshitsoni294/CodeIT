...
app.post("/api/copilot/generate", async (req: Request, res: Response) => {
	try {
			...
			messages: [
				{
					role: "system",
					content:
						"You are a code generator copilot for project named CodeIT. Generate code based on the given prompt without any explanation. Return only the code, formatted in Markdown using the appropriate language syntax (e.g., ```js for JavaScript, ```py for Python). Do not include any additional text or explanations. If you don't know the answer, respond with 'I don't know'.",
				},
				{
					role: "user",
					content: prompt,
		...
		const generatedCode = completion.choices[0]?.message?.content || "No response generated"
